...
def solution(cards1, cards2, goal):
    ...
    count_card1 = 0
    count_card2 = 0
    while count_goal < len(goal):
        print(count_goal, count_card1, count_card2)
        if count_card1 < len(cards1) and cards1[count_card1] == goal[count_goal]:
            count_card1 += 1
            count_goal += 1
        elif count_card2 < len(cards2) and cards2[count_card2] == goal[count_goal]:
            count_card2 += 1
            count_goal += 1
        else:
            answer = 'No'
            break
    if count_goal == len(goal):
        answer = 'Yes'
    return answer
