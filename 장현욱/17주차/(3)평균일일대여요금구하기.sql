...
/* 해결해야 하는 요구사항
자동차 종류 'SUV'인 자동차 선택 -> WHERE 절 이용
평균 일일 대여 요금 계산 -> AVG() 이용
소수 첫 번째 자리에서 반올림 -> ROUND() 이용
컬럼명 지정 -> AS 이용
*/

SELECT ROUND(AVG(DAILY_FEE), 0) AS AVERAGE_FEE /* 평균값을 출력해줘 */
FROM CAR_RENTAL_COMPANY_CAR  /*CAR_RENTAL_COMPANY_CAR테이블 안에서*/
WHERE CAR_TYPE = 'SUV'  /* SUV타입의 */