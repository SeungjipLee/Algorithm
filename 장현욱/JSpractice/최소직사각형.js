...
// 3	60	30
// 4	80	40
// 가장 긴 가로 길이와 세로 길이가 각각 
// 80, 70이기 때문에 80(가로) x 70(세로) 크기의 지갑을 만들면 모든 명함들을 
// 수납할 수 있습니다. 하지만 2번 명함을 가로로 눕혀 수납한다면 
...
function solution(sizes) {
  ...
  let best = []
  let second = []
  for(let now of sizes) {
    best.push(Math.max(...now))
    second.push(Math.min(...now))
  }
  answer = (Math.max(...best)) * (Math.max(...second))
  return answer;
}