// △△ 게임대회가 개최되었습니다. 이 대회는 N명이 참가하고, 
// 토너먼트 형식으로 진행됩니다. N명의 참가자는 각각 1부터 N번을 차례대로 배정받습니다. 
...
// 몇 번째 라운드에서 만나는지 궁금해졌습니다. 게임 참가자 수 N, 참가자 번호 A, 
// 경쟁자 번호 B가 함수 solution의 매개변수로 주어질 때, 
...
function solution(n,a,b)
{
    ...
    while (member.length > 0) {
        if (member.length == 1) {
            nextMember.push(member.shift())
        }
        else{
        const [x,y] = [member.shift(), member.shift()]
        if (x === a && y === b || x === b && y === a) {
            return answer
        } else if (x == a || y == a) {
            nextMember.push(a)
